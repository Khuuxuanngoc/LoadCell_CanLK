...
<script>
    ...
    ws.onmessage = function(e) {
        ...
        document.getElementById('gt2').innerHTML = 'Raws: ' + d.gt2;
    };
...
void LoadcellServer::begin(ButtonCallback cb) {
    ...
    // Truyền cả server và con trỏ tới đối tượng WebSocket (_ws) vào TaskOTA
    TaskOTA::init(&_server, &_ws);
...
void LoadcellServer::updateData(float gt1, long gt2) {
    // CHẶN: Nếu đang OTA thì không gửi gì cả để tránh nghẽn CPU
    if (TaskOTA::isUpdating) return;
